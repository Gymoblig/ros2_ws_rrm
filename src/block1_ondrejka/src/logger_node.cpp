...
private:
    // pomocna funkcia pre transformaciu
    ...
    void js_cb(const sensor_msgs::msg::JointState::SharedPtr msg) { 
        ...
        if (last_js_.size() >= 3) {
            ...
            double q1 = last_js_[0];
            double q2 = last_js_[1];
            double q3 = last_js_[2];

            double l2 = 0.203;
            double l3 = 0.203;

            // A1: a=0, alpha=90, d=0, theta=180 + q1
            Eigen::Matrix4d A1 = get_transform(deg2rad(180.0) + q1, 0.0, 0.0, deg2rad(90.0));
            // A2: a=0.203, alpha=0, d=0, theta=90 + q2
            Eigen::Matrix4d A2 = get_transform(deg2rad(90.0) + q2, 0.0, l2, 0.0);
            // A3: a=0.203, alpha=0, d=0, theta=0 + q3
            Eigen::Matrix4d A3 = get_transform(q3, 0.0, l3, 0.0);
            ...
            t.header.frame_id = "base_link";         // Odkiaľ meriame (stred robota)
            t.child_frame_id = "tool0_calculated";   // Čo meriame (vypočítaný koniec)
